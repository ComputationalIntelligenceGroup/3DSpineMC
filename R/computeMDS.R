...
#' @param ndim number of dimension of the projection
#'
...
#' @param colors (optional) a matrix of rgb colors where each row defines the colors of a cluster
#'
#' @return None
#'
#' @examples
...
plotMDS<-function(model, MDS, colors=NULL)
{
  if(ncol(MDS$centers)>3 | ncol(MDS$centers)<2){
    stop("Too high dimension. It must be 2D or 3D")
  }
  ...
  if(ncol(MDS$centers)==3)
  {
    ...
    text3d(MDS$centers[,1]+0.1, MDS$centers[,2]+0.1, MDS$centers[,3]+0.1, c(1:model$G),col=rgb(centColors[,1],centColors[,2],centColors[,3],maxColorValue=255),cex=2)
    bg3d(col="white")
    axes3d(lwd=3)
  }
  ...
  if(ncol(MDS$centers)==2)
  {
    ...
    text(MDS$centers[,1]+0.35, MDS$centers[,2]-0.25, c(1:model$G),col=rgb(centColors[,1],centColors[,2],centColors[,3],maxColorValue=255),cex=2)
  }
}
